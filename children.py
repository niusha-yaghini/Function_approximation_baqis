...
def making_children(list_of_parents, type_of_selection, k, pc, pm1, pm2, list_x, actual_y):
    ...
    mutation_different_value_and_neighbors(children, pm1, pm2, list_x, actual_y)
...
def finding_best_child_in_mutation_iteration(child_mutations):
    best_mse = float('inf')
    best_child = None
    for child in child_mutations:
        mse = Chromosome._mse()
        
def mutation_different_value_and_neighbors(children, pm1, pm2, list_x, actual_y):
